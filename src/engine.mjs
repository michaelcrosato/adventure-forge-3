...
function actionEvent(world, state, actionId, nextState = state) {
  ...
  if (
    ...
    return state.flags.includes("boat_signaled")
      ...
      : "Mooring is secure; the boat will hold. The keeper's penciled instruction says: signal the boat to hold position; the signal only holds position, and a later radio check confirms the channel (the margin labels old shorthand \"signal the boat for a confirmed channel\" as incomplete).";
  }
  if (
    actionId === "enter_house" &&
    state.inventory.includes("lantern") &&
