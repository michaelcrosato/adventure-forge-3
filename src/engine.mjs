...
function actionEvent(world, state, actionId, nextState = state) {
  ...
  if (
    ...
    return state.flags.includes("boat_signaled")
      ...
      : "Mooring is secure; the boat will hold. The keeper's penciled instruction says: signal the boat to hold position; the signal only holds position, and a later radio check confirms the channel (the margin labels old shorthand \"signal the boat for a confirmed channel\" as incomplete). Only that old line is crossed out.";
  }
  if (
    actionId === "enter_house" &&
    state.inventory.includes("lantern") &&
  ...
  if (actionId === "signal_boat") {
    ...
    if (hasLog && state.turn > 4) {
      return "Ring the bell; supply boat holds position. The tide chart and oil are still waiting.";
    }
  ...
  if (actionId === "check_tower_radio") {
    ...
    if (
      ...
      return "Channel is clear; the sheltered confirmed-channel finish is ready.";
    }
    if (
      state.flags.includes("wick_trimmed") &&
      state.flags.includes("lens_aligned") &&
...
export function observation(world, state, event = null) {
  ...
  const towerRadioActionReachable =
    state.room === "tower" &&
    (state.flags.includes("supply_return_used") || state.turn === world.maxTurns - 1) &&
    !state.flags.includes("radio_checked")
  ...
  const actions = clueFirstActions.map((id) => [
    ...
    id === "secure_mooring" &&
      ...
      : id === "wait_for_horn" &&
          ...
          state.flags.includes("wick_trimmed") &&
          state.flags.includes("lens_aligned")
        ? "Wait for the horn for a timing bonus; costs one turn; optional preparation can still follow before lighting"
      : id === "wait_for_horn" &&
          state.room === "tower" &&
          state.turn < world.maxTurns - 1 &&
          state.flags.includes("radio_checked") &&
...
export function modelTurnInput(world, view) {
  ...
  if (typeof modelText === "string") {
    ...
    if (
      ...
      visibleActionIds.has("go_workshop") &&
      typeof view.event === "string" &&
      view.event.startsWith("The channel is clear;")
    ) {
      modelText = "The channel is clear. Take the oil or enter the workshop when ready.";
    }
  ...
  if (
    ...
    !visibleFacts.some((fact) => /Tide chart:/i.test(fact)) &&
    typeof view.event === "string" &&
    view.event.startsWith("Ring the bell; supply boat holds position. The tide chart and oil are still waiting.")
  ) {
    modelText = "The boat holds. The tide chart and oil are still waiting; choose what to investigate next.";
  }
  if (
    view.at?.[0] === "keeper_room" &&
    hasMooringRecovery &&
  ...
  if (
    ...
    modelLastEvent = "The channel is clear.";
  }
  if (
    view.at?.[0] === "workshop" &&
    typeof view.event === "string" &&
