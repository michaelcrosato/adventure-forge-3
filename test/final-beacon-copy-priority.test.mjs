...
test("the final beacon model input prefers a specific finish cue over the generic deadline", async () => {
  const world = await loadWorld();
  const state = replayActions(world, 4, [
    "take_lantern",
    "enter_house",
    "read_log",
    "study_tide_chart",
  ...
  assert.match(view.text, /Last turn: light now/i);
