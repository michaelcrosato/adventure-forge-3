...
test("marked tide and a tuned beam get the strongest unsignaled finish", async () => {
  const world = await loadWorld();
  const tower = replayActions(world, 1103, [
    "take_lantern",
    "secure_mooring",
    "enter_house",
    "study_tide_chart",
  ...
  assert.equal(actions.includes("light_moored_tide_tuned_beacon"), true);

  const ending = step(world, tower, "light_moored_tide_tuned_beacon");
